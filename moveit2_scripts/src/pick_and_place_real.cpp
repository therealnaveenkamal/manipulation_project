...
int main(int argc, char **argv) {
  ...
  */
  ...
  /*
  PICKING IT FROM THE BLOCK!

  RCLCPP_INFO(LOGGER, "Pregrasp Position");
  joint_group_positions_arm[0] = -1.349301;  // Shoulder Pan
  joint_group_positions_arm[1] = -2.179562;  // Shoulder Lift
  joint_group_positions_arm[2] = -1.0171;   // Elbow
  joint_group_positions_arm[3] = -1.4719;   // Wrist 1
  joint_group_positions_arm[4] = -4.710;   // Wrist 2
  joint_group_positions_arm[5] = -4.407; // Wrist 3
  move_group_arm.setJointValueTarget(joint_group_positions_arm);
...
/*
  RCLCPP_INFO(LOGGER, "Pregrasp Position");
  joint_group_positions_arm[0] = -1.226637;  // Shoulder Pan
  joint_group_positions_arm[1] = -1.471964;  // Shoulder Lift
  joint_group_positions_arm[2] = -1.794956;   // Elbow
  joint_group_positions_arm[3] = -1.471964;   // Wrist 1
  joint_group_positions_arm[4] = -4.709986;   // Wrist 2
  joint_group_positions_arm[5] = 0.122664; // Wrist 3
  move_group_arm.setJointValueTarget(joint_group_positions_arm);
  moveit::planning_interface::MoveGroupInterface::Plan my_plan;
  bool success = (move_group_arm.plan(my_plan) == moveit::core::MoveItErrorCode::SUCCESS);
  if (success) {
...
*/
  ...
  RCLCPP_INFO(LOGGER, "Pregrasp Position");
  joint_group_positions_arm[0] = 3.869699;  // Shoulder Pan
  joint_group_positions_arm[1] = -1.76128;  // Shoulder Lift
...
/* 
geometry_msgs::msg::Pose final_pose = move_group_arm.getCurrentPose().pose;
final_pose.position.x = 0.343;
final_pose.position.y = 0.132;
move_group_arm.setPoseTarget(final_pose);
moveit::planning_interface::MoveGroupInterface::Plan final_plan;
bool success_final = (move_group_arm.plan(final_plan) == moveit::core::MoveItErrorCode::SUCCESS);
if(success_final) {
    move_group_arm.execute(final_plan);
} else {
...
moveit::planning_interface::MoveGroupInterface::Plan my_plan_gripper;
  RCLCPP_INFO(LOGGER, "Open Gripper!");
    joint_group_positions_gripper[2] = 0.2; //0.655
  ...
  RCLCPP_INFO(LOGGER, "Approach to object!");
    ...
    move_group_arm.execute(trajectory_retreat);
      ...
      bool success_gripperv1 = (move_group_gripper.plan(my_plan_gripper) ==
                         moveit::core::MoveItErrorCode::SUCCESS);
      move_group_gripper.execute(my_plan_gripper);
